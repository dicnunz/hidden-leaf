#!/usr/bin/env python3
"""Package the Hidden Leaf Godot project as a self-contained local Apple Silicon app.

This adapts the proven direct-engine bundle layout. The actual Godot process is
CFBundleExecutable, and discovers its sibling Resources/HiddenLeafExplorer.pck.
...
EXTRA_ASSETS = {
    "tree_small_02": "https://polyhaven.com/a/tree_small_02",
    "grass_medium_01": "https://polyhaven.com/a/grass_medium_01",
    "kloofendal_48d_partly_cloudy_puresky": "https://polyhaven.com/a/kloofendal_48d_partly_cloudy_puresky",
}
...
def credits_from_manifest(path: Path) -> tuple[str, dict[str, str]]:
    ...
    lines = [
        ...
        "[Ramen Ichiraku](https://naruto.fandom.com/wiki/Ramen_Ichiraku), "
        "[Yamanaka Flowers](https://naruto.fandom.com/wiki/Yamanaka_Flowers), "
        "[Konohagakure](https://naruto.fandom.com/wiki/Konohagakure), and "
        "[Hokage Residence](https://naruto.fandom.com/wiki/Hokage_Residence). "
        "These references remain copyrighted to their respective owners. The reconstruction uses "
        "the pre-Pain Shippuden era with five Hokage portraits; unshown geography and dimensions are inferred.", "",
        "Built with [Godot Engine](https://godotengine.org), distributed under the MIT license.",
        "The app includes the engine license and its third-party notices in Contents/Resources/Licenses.", "",
        "Environment textures, vegetation and sky: [Poly Haven](https://polyhaven.com), CC0 1.0.", "",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--qa-dir", type=Path, default=WORK / "qa" / "package_explorer")
    parser.add_argument("--icon", type=Path, default=ROOT / "docs" / "AppIcon.icns", help="Optional .icns file for the native app bundle")
    ...
    build_parent = WORK / "package_explorer"
    ...
    try:
        ...
        if args.smoke_test:
            ...
            with tempfile.TemporaryDirectory(prefix="hidden-leaf-smoke-") as empty:
